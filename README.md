...
Build a question-answering chatbot that can answer questions related to supermarket sales by creating SQL queres to get the needed information from database.

Main Objectives
- Prepare the SQL tables from a kaggle dataset that will be used by the chatbot to answer users' questions.
- Design and test the prompt and build tools for the chatbot, in order to generate accurate query to get needed information.
- Build a questions answering bot that can process the information and find the answer based on contexts for the user.
